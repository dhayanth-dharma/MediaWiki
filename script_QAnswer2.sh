...
request=$(curl -X POST 'http://qanswer-core1.univ-st-etienne.fr/api/user/signin' -H 'Content-Type: application/json' -d '{"usernameOrEmail": "dhayanthdharma", "password":"MarioDhaya123@"}')
echo $request
token=$(echo $request | jq -r '.accessToken')
echo $token
#upload and index the dataset to QAnswer
#echo "Uploading ..."
#curl -X POST 'https://qanswer-core1.univ-st-etienne.fr/api/dataset/upload?dataset=test' -H "Authorization: Bearer $token" -F file=@dump.nt
echo "Updating"
curl -X POST 'https://qanswer-core1.univ-st-etienne.fr/api/dataset/edit?dataset=test' -H "Authorization: Bearer $token" -F file=@dump.nt
echo "Indexing ..."
echo "Indexing ..."
curl -X POST 'https://qanswer-core1.univ-st-etienne.fr/api/dataset/index' --data '{"dataset": "test", "properties": ["http://purl.org/dc/elements/1.1/title","https://schema.org/name","http://www.w3.org/2004/02/skos/core#prefLabel","http://www.w3.org/2004/02/skos/core#altLabel","http://www.w3.org/2000/01/rdf-schema#label","http://www.wikidata.org/prop/direct/P1549","http://purl.org/dc/terms/identifier","http://purl.org/dc/terms/title","https://linkedopendata.eu/prop/direct/P150"], "type": ["https://linkedopendata.eu/prop/direct/P35","https://linkedopendata.eu/prop/direct/P1845"], "subclass": ["https://linkedopendata.eu/prop/direct/P302","https://linkedopendata.eu/prop/direct/P1845"]}' -H 'Content-Type: application/json' -H "Authorization: Bearer $token"
